<x-app-layout>
    <div class="flex items-center justify-center min-h-screen bg-[#111827]">
        <div class="w-full max-w-lg bg-gray-900 p-6 rounded-lg shadow-lg">
            <h2 class="text-white text-2xl font-semibold mb-6 text-center">Edit Product</h2>
            ...
            <form action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <!-- Image Upload -->
                <div class="mb-4">
                    <label for="image" class="block text-gray-300 text-sm font-medium mb-2">Change Image</label>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="4" class="w-full p-3 border rounded-md bg-gray-800 text-white focus:ring focus:ring-blue-500">{{ old('description', $product->description) }}</textarea>
                    @error('description') 
                        <span class="text-red-500 text-sm">{{ $message }}</span> 
                    @enderror
                </div>
                ...
                <!-- Submit Button -->
                <div class="mt-6">
                    <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-md transition">Update Product</button>
                </div>
            </form>
        </div>
    </div>
